...
async function addRaceResults(raceResults, firstRace = false) {
    ...
    let oldGeneralClassification = await GeneralClassificationModel.find({}).select('runner races points racePositions');
    ...
    if (firstRace) {
        ...
        raceResults.forEach(result => {
            ...
            if (returningRunner) {
                ...
                returningRunner.racePositions.push(result.position);
...
async function removeWorstResult() {
    let finalGeneralClassification = []
    let currentGeneralClassification = await GeneralClassificationModel.find({}).select('runner races points racePositions');
    // console.log(currentGeneralClassification);

    currentGeneralClassification.forEach(result => {
        if (result.racePositions.length === 4) {
    ...
    // Finally, insert the general classification after the first race into the MongoDB collection
    // console.log(generalClassification)
    const createGeneralClassification = await GeneralClassificationModel.create(finalGeneralClassification);
}
...
async function generateResults() {
    ...
    await recalculateGeneralClassificationResults();
    await removeWorstResult();
}
