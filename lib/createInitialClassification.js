...
// Create the general classification
const FinalStandingsModel = mongoose.model('FinalStandings', generalClassificationSchema);
// console.log(CurrentStandingsModel)

// Create the general classification after each race to be able to view how classification changed from race to race
// const FiveRacesStandingsModel = mongoose.model('FiveRacesStandings', generalClassificationSchema)
...
const sixthRaceResults = await Race.find({name: "Qx5.xlsx"});
...
let sixthRaceResultsProper = []

sixthRaceResults.forEach(result => {
    for (var i = 0; i < result.results.length; i++)
    sixthRaceResultsProper.push(result.results[i]);
})

let racesToUpdate = [firstRaceResultsProper, secondRaceResultsProper, thirdRaceResultsProper, fourthRaceResultsProper, fifthRaceResultsProper, sixthRaceResultsProper]
// console.log(racesToUpdate)

async function addRaceResults(raceResults, firstRace = false) {
    // console.log(classification)
    let generalClassification = [];
    let oldGeneralClassification = await FinalStandingsModel.find({}).select('runner races points category generalPosition racePositions');
    ...
    const deleteManyResult = await FinalStandingsModel.deleteMany({});
    // Finally, insert the general classification after the first race into the MongoDB collection
    // console.log(generalClassification)
    const createGeneralClassification = await FinalStandingsModel.create(generalClassification);
}

async function recalculateGeneralClassificationResults(numberOfRaces) {
    
    let currentGeneralClassification = await FinalStandingsModel.find({}).select('runner gender races points');
...
async function removeWorstResult(numberOfRaces) {
    ...
    let currentGeneralClassification = await FinalStandingsModel.find({}).select('runner races points category generalPosition racePositions');
    // console.log(currentGeneralClassification);
    ...
    // Finally, insert the general classification after the first race into the MongoDB collection
    // console.log(generalClassification)
    const createGeneralClassification = await FinalStandingsModel.create(finalGeneralClassification);
}

async function correctGeneralPositions() {
    let generalClassification = [];

    let currentStandings = await FinalStandingsModel.find({}).select('runner races points category generalPosition racePositions');
...
async function generateResults() {
    ...
    // await addRaceResults(sixthRaceResultsProper, false, SixthRaceClassificationModel);
    ...
    await addRaceResults(sixthRaceResultsProper);

    // await recalculateGeneralClassificationResults(6);
    // await correctGeneralPositions()
    // await removeWorstResult(6);
}

// generateResults();

// recalculateGeneralClassificationResults(6)
