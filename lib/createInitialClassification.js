...
// Create the general classification after 4 races to be able to view how classification changed from race to race
...
let racesToUpdate = [firstRaceResultsProper]
// console.log(racesToUpdate)

async function addRaceResults(raceResults, firstRace = false) {
    // console.log(classification)
    let generalClassification = [];
    let oldGeneralClassification = await OneRacesStandingsModel.find({}).select('runner races points category generalPosition racePositions');
    ...
    const deleteManyResult = await OneRacesStandingsModel.deleteMany({});
    // Finally, insert the general classification after the first race into the MongoDB collection
    // console.log(generalClassification)
    const createGeneralClassification = await OneRacesStandingsModel.create(generalClassification);
}

async function recalculateGeneralClassificationResults(numberOfRaces) {
    
    let currentGeneralClassification = await OneRacesStandingsModel.find({}).select('runner gender races points');
...
async function removeWorstResult(numberOfRaces) {
    ...
    let currentGeneralClassification = await OneRacesStandingsModel.find({}).select('runner races points generalPosition racePositions');
    // console.log(currentGeneralClassification);
    ...
    // Finally, insert the general classification after the first race into the MongoDB collection
    // console.log(generalClassification)
    const createGeneralClassification = await OneRacesStandingsModel.create(finalGeneralClassification);
}

async function correctGeneralPositions() {
    let generalClassification = [];

    let currentStandings = await OneRacesStandingsModel.find({}).select('runner races points category generalPosition racePositions');
...
async function generateResults() {
    ...
    // await recalculateGeneralClassificationResults(3);
